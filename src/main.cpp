...
uint256 const hashGenesisBlock("0x4df417b777f0e78535850a55381de7e0c952ae31335116df4e54c50a175c4831");
...
const string strMessageMagic = "Privatecoin Signed Message:\n";
...
// after what block number should we change to the second difficulty adjustment targets
static const int64 nSecondTargetAfterBlock = 100;

// second targets
static const int64 nSecondTargetTimespan = 3.5 * 24 * 60 * 60; // Privatecoin: 3.5 days
static const int64 nSecondTargetSpacing = 2.5 * 60; // Privatecoin: 2.5 minutes
static const int64 nSecondInterval = nSecondTargetTimespan / nSecondTargetSpacing;

// testing:
//static const int64 nSecondTargetTimespan = 40 * 60; // Privatecoin: 40 minutes
//static const int64 nSecondTargetSpacing = 1 * 60; // Privatecoin: 1 minute
//static const int64 nSecondInterval = nSecondTargetTimespan / nSecondTargetSpacing;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    int64 targetTimespan;
    int64 targetSpacing;
    int64 interval;

    if (pindexLast->nHeight + 1 > nSecondTargetAfterBlock) {
        targetTimespan = nSecondTargetTimespan;
        targetSpacing = nSecondTargetSpacing;
        interval = nSecondInterval;
    } else {
        targetTimespan = nTargetTimespan;
        targetSpacing = nTargetSpacing;
        interval = nInterval;
    }
    ...
    if (nActualTimespan < targetTimespan*difficulty_increase_max/100)
        nActualTimespan = targetTimespan*difficulty_increase_max/100;
    if (nActualTimespan > targetTimespan*difficulty_decrease_max/100)
        nActualTimespan = targetTimespan*difficulty_decrease_max/100;
    ...
    printf("targetTimespan = %"PRI64d"    nActualTimespan = %"PRI64d"\n", targetTimespan, nActualTimespan);
...
{
    // Privatecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "aaaaaaaaaaa";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1393395149;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084658606;

        if (fTestNet)
        {
            block.nTime    = 1393395198;
        ...
        assert(block.hashMerkleRoot == uint256("0x4da715ee7947d7543d7364a7c5206cc1dd8ede24b107e7707fa7ac34d1c28746"));
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc2, 0xb8, 0xdd }; // Privatecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PrivatecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PrivatecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PrivatecoinMiner(CWallet *pwallet)
{
    printf("PrivatecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("privatecoin-miner");
    ...
    try { loop {
        ...
        printf("Running PrivatecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PrivatecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PrivatecoinMiner, pwallet));
}
